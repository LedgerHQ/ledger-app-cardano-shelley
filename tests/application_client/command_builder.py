...
from input_files.derive_native_script import NativeScriptParamsDeviceOwnedPubkey, NativeScriptParamsThirdPartyPubkey
from input_files.derive_native_script import NativeScriptParamsInvalid
...
class CommandBuilder:
    ...
    def derive_script_add_simple(self, script: NativeScript) -> bytes:
        ...
        if script.type == NativeScriptType.PUBKEY_DEVICE_OWNED:
            ...
            assert isinstance(script.params, NativeScriptParamsDeviceOwnedPubkey)
            data += pack_derivation_path(script.params.path)
        elif script.type == NativeScriptType.PUBKEY_THIRD_PARTY:
            data += int(0).to_bytes(1, "big")
            data += self._derive_script_pubkey(script.type)
            assert isinstance(script.params, NativeScriptParamsThirdPartyPubkey)
            data += bytes.fromhex(script.params.keyHashHex)
    ...
    def derive_script_add_complex(self, script: NativeScript) -> bytes:
        ...
        if script.type in (NativeScriptType.ALL, NativeScriptType.ANY):
            assert isinstance(script.params, (NativeScriptParamsAll, NativeScriptParamsAny))
