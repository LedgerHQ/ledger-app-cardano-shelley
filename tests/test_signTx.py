...
from input_files.signTx import SignTxTestCase, AssetGroup, TxOutputDestinationType
from input_files.signTx import ThirdPartyAddressParams, DeriveAddressTestCase, TxAuxiliaryDataType, TxAuxiliaryDataCIP36
from input_files.signTx import TransactionSigningMode, CertificateType, CredentialType, DRepParamsType, VoterType
from input_files.signTx import MAX_SIGN_TX_CHUNK_SIZE, TxOutputBabbage, DatumType
from input_files.signTx import testsShelleyWithCertificates, testsConwayWithCertificates
from input_files.signTx import testsConwayWithoutCertificates, testsConwayVotingProcedures
from input_files.signTx import testsMultisig, testsMary, testsCatalystRegistration, testsMultidelegation
from input_files.signTx import testsBabbageTrezorComparison, testsAlonzoTrezorComparison, testsAllegra, testsAlonzo, testsBabbage
...
from input_files.signTx import singleAccountRejectTestCases, collateralOutputRejectTestCases #, certificateRejectTestCases
...
@pytest.mark.parametrize(
    ...
    testsShelleyWithCertificates + testsConwayWithCertificates + \
    testsMultisig + testsMary + \
    testsConwayWithoutCertificates + testsConwayVotingProcedures + \
    testsCatalystRegistration + testsAlonzo + testsBabbage + \
    testsMultidelegation + testsAllegra + testsAlonzoTrezorComparison + \
...
def test_signTx(firmware: Firmware,
    ...
    _signTx_addCollateralOutputs(firmware, navigator, client, testCase)
...
def _signTx_setAuxiliaryData(firmware: Firmware,
    ...
    if testCase.tx.auxiliaryData is None:
        return False
    with client.sign_tx_aux_data_hash(testCase.tx.auxiliaryData):
    ...
    if testCase.tx.auxiliaryData.params.voteKeyHex is not None or \
        testCase.tx.auxiliaryData.params.voteKeyPath is not None:
        if firmware.is_nano:
        ...
        raise NotImplementedError("Delegations are not supported")
        ...
        with client.sign_tx_certificate(certificate):
            if firmware.is_nano:
                ...
                if testCase.signingMode == TransactionSigningMode.MULTISIG_TRANSACTION:
                    ...
                    moves += [NavInsID.BOTH_CLICK]
                if certificate.coldCredential is not None and certificate.coldCredential.type != CredentialType.KEY_PATH:
                    moves += [NavInsID.RIGHT_CLICK]
                if certificate.hotCredential is not None and certificate.hotCredential.type != CredentialType.KEY_PATH:
                    moves += [NavInsID.RIGHT_CLICK]
                if certificate.type == CertificateType.AUTHORIZE_COMMITTEE_HOT:
                    moves += [NavInsID.BOTH_CLICK]
                if certificate.dRep is not None:
                    if certificate.dRep.type in (DRepParamsType.KEY_HASH, DRepParamsType.SCRIPT_HASH):
                        moves += [NavInsID.RIGHT_CLICK]
                    moves += [NavInsID.BOTH_CLICK]
                if certificate.dRepCredential is not None and certificate.type != CertificateType.DREP_DEREGISTRATION:
                    moves += [NavInsID.BOTH_CLICK]
                if certificate.deposit is not None:
                    moves += [NavInsID.BOTH_CLICK]
                if certificate.anchor is not None:
                    if len(certificate.anchor.url) > 50:
...
def _signTx_addCollateralOutputs(firmware: Firmware,
    ...
    # Send CONFIRM
    response = client.sign_tx_collateral_output_confirm()
    # Check the status
    assert response and response.status == Errors.SW_SUCCESS
...
def _gatherWitnessPaths(testCase: SignTxTestCase) -> List[str]:
    ...
    if testCase.signingMode != TransactionSigningMode.MULTISIG_TRANSACTION:
            ...
            for cert in testCase.tx.certificates:
                if cert.type in (CertificateType.STAKE_DEREGISTRATION,
                    ...
                    if cert.stakeCredential.type == CredentialType.KEY_PATH:
                        witnessPaths.append(cert.stakeCredential.keyValue)

                elif cert.type in (CertificateType.AUTHORIZE_COMMITTEE_HOT,
                                   CertificateType.RESIGN_COMMITTEE_COLD):
                    if cert.coldCredential is not None and cert.coldCredential.type == CredentialType.KEY_PATH:
                        witnessPaths.append(cert.coldCredential.keyValue)

                elif cert.type in (CertificateType.DREP_REGISTRATION,
                                   CertificateType.DREP_DEREGISTRATION,
                                   CertificateType.DREP_UPDATE):
                    assert cert.dRepCredential is not None
                    if cert.dRepCredential.type == CredentialType.KEY_PATH:
                        witnessPaths.append(cert.dRepCredential.keyValue)

                elif cert.type == CertificateType.STAKE_POOL_RETIREMENT:
                    assert cert.poolKey is not None
                    witnessPaths.append(cert.poolKey)

                elif cert.type == CertificateType.STAKE_POOL_REGISTRATION:
                    raise NotImplementedError("Stake Pool Registration is not supported")
        ...
        for signer in testCase.tx.requiredSigners:
            if signer.type == CredentialType.KEY_PATH:
...
@pytest.mark.parametrize(
    ...
    withdrawalRejectTestCases + witnessRejectTestCases + \
    testsInvalidTokenBundleOrdering + singleAccountRejectTestCases + \
