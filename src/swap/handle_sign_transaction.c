...
bool swap_check_destination_validity(tx_output_destination_t* destination) {
    ...
    switch (destination->type) {
        case DESTINATION_THIRD_PARTY:
            humanReadableAddress(destination->address.buffer,
        ...
        case DESTINATION_DEVICE_OWNED:
            rawAddressBuffer[nbBytes++] = destination->params->type;
            rawAddressBuffer[nbBytes++] = destination->params->networkId;
            ...
            humanReadableAddress((const uint8_t*) rawAddressBuffer,
                                 nbBytes,
                                 rawAddressHuman,
                                 sizeof(rawAddressHuman));
